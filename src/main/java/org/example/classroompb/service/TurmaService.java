...
public class TurmaService {
    ...
    public TurmaService(TurmaRepository repository, UsuarioService usuarioService, DisciplinaService disciplinaService) {
    ...
    /**
     * RF10: O coordenador deve poder ofertar turmas para uma disciplina em um período letivo
     * RF11: Cada turma deve possuir professor responsável, limite de vagas, horário e sala
     */
    public Turma ofertarTurma(String codigoDisciplina, String matriculaProfessor, 
                              PeriodoLetivo periodoLetivo, int limiteVagas, 
    ...
    /**
     * RN03: Uma turma não pode ultrapassar o número máximo de vagas
     * RN01: Um aluno não pode se matricular duas vezes na mesma turma
     */
        ...
        for (Turma turmaExistente : turmasDoAluno) {
            if (horarioConflita(turmaExistente.getHorario(), horarioNova)) {
                throw new IllegalArgumentException(
                    "Aluno possui conflito de horário com turma " + turmaExistente.getCodigo() + 
                    " (" + turmaExistente.getHorario() + "). (RN02)"
                );
            }
        }
    }
        ...
        for (Turma turmaExistente : turmasProfessor) {
            if (horarioConflita(turmaExistente.getHorario(), horarioNova)) {
                throw new IllegalArgumentException(
                    ...
                    "Conflito com turma " + turmaExistente.getCodigo() + 
                    " (" + turmaExistente.getHorario() + "). (RN06)"
                );
            }
        }
    }
    ...
    private void validarCampos(String codigoDisciplina, String matriculaProfessor,
                               PeriodoLetivo periodoLetivo, int limiteVagas,
                               String horario, String sala) {
    ...
    private String gerarCodigoTurma(String codigoDisciplina, PeriodoLetivo periodoLetivo) {
        ...
        long contagem = turmas.stream()
                .filter(t -> t.getDisciplina().getCodigo().equals(codigoDisciplina) &&
                            t.getPeriodoLetivo().getIdentificador().equals(periodoLetivo.getIdentificador()))
                .count();
